...
public class CryptByteBufferTest {
    ...
    static {
//        Security.addProvider(JceLoader.NSS);
        Security.addProvider(JceLoader.SUN);
        Security.addProvider(JceLoader.SunJCE);
        Security.addProvider(JceLoader.BouncyCastle);
    }
    ...
    public void testSuccessfulRoundTripByteArray() throws GeneralSecurityException {
        ...
        test((plain, crypt) -> {
            byte[] decipheredtext = crypt.decryptCopy(crypt.encryptCopy(Hex.decode(plain)));
            assertArrayEquals("CryptByteBufferType: " + crypt.type.name(),
                    Hex.decode(plain), decipheredtext);
        });
    }

    void test(CryptTest t) {
        for (int i = 0; i < cipherTypes.length; i++) {

            CryptByteBuffer crypt = null;
            ...
            if (err == null) {
                t.accept(plainText[i], crypt);
                System.out.println(type + " OK");
            } else {
                System.err.println(type + " FAIL");
            }
    ...
    interface CryptTest {
        public void accept(String plainText, CryptByteBuffer crypt);
    }
    ...
    public void testEncryptDirectByteBuffer() throws GeneralSecurityException {
        test((pt, crypt) -> {
            byte[] origPlaintext = Hex.decode(pt);
            ByteBuffer plaintext = ByteBuffer.allocateDirect(origPlaintext.length);
            plaintext.clear();
            plaintext.put(origPlaintext);
            plaintext.clear();
