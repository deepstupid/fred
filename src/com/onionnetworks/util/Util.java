...
//        } while(zeroLength < len);
    ...
    public static boolean arraysEqual(int[] arr1, int start1,
        ...
        //return Arrays.equals(arr1, start1, start1+len, arr2, start2, start2+len);
    ...
    public static boolean arraysEqual(long[] arr1, int start1,
        ...
        //return Arrays.equals((long[])arr1, start1, start1+len, (long[])arr2, start2, start2+len);
    ...
    public static final void arraycopy(char[] chars, int charOff,
        ...
        for (int i = charOff; i < loopMax; i++) {
            char c = chars[i];
            bytes[indexCounter++] = (byte) ((c & 0xFF00) >> 8);
            bytes[indexCounter++] = (byte) (c & 0xFF);
        }
    ...
    public static final void arraycopy(byte[] bytes, int byteOff,
                                       ...
                                       int numBytes) {
        System.arraycopy(bytes, byteOff, chars, charOff, numBytes);
...
//        }
    ...
    public static String bytesToHex(byte[] in, int off, int len) {
        ...
        for (int i = j*2; i < len; ) {
            byte ii = in[j++];
            out[i++] = hexDigit[(0xF0 & ii) >> 4]; // high nybble
            out[i++] = hexDigit[0xF & ii]; // low nybble
        }
